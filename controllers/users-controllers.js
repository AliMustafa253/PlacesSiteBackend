...
const signup = async (req, res, next) => {
  ...
  if (existingUser) {
    const error = new HttpError("User exists already, please log in instead.");
    return next(error);
  }
  ...
  try {
    ...
    const error = new HttpError(
      "Signing up failed user not saved, please try again later.",
      500
    );
    return next(error);
  }
  ...
  try {
    ...
    const error = new HttpError(
      "Signing up failed jwt, please try again later.",
      500
    );
    return next(error);
  }
...
const login = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError(
      "Could not log in user, credentials seems wrong",
      500
    );
    return next(error);
  }
